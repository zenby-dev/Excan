...
function Pause( name )
	...
	if ( Timer[name].Status == RUNNING ) then
		Timer[name].Diff = os.clock() - Timer[name].Last
...
function Check()
	...
	for key, value in pairs( Timer ) do
		...
		if ( value.Status == PAUSED ) then
			...
			value.Last = os.clock() - value.Diff
			
		elseif ( value.Status == RUNNING and ( value.Last + value.Delay ) <= os.clock() ) then
...
function Simple( delay, func, ... )
	...
	new_timer.Finish = os.clock() + delay
...
hook.Add( "Update", "CheckTimers", Check )